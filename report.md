...
**Injected-pair POC on GPT-2-small layer-6 activations (36 runs, `experiments/poc_weighted_gpt2.py`).** Absorption *transfers to real background*: vanilla SAEs recover the injected child cleanly at ε = 0.05 (cos ≈ 0.90, positive control) and degrade to split/absorbed geometry at ε = 0.002. The **oracle-free residual-weighting estimator is the best condition overall** — rescuing 3/4 seeds at ε = 0.002 (cos 0.84–0.85) and matching vanilla at large ε — while **exact inverse-density oracle weights *underperform* at moderate ε** (max-cos ≈ 0.70 vs vanilla's 0.90 at ε = 0.05): the variance amplification §12 warned about is real on real data, and the clipped residual heuristic turns out to be the practically correct form of the remedy, not a compromise. Scored against the pre-registered predictions: P1 (transfer) confirmed; P2 (oracle works everywhere) **refuted in its strong form** — oracle weighting helps only where absorption actually bites; P3 (residual works) confirmed at 3/4.

**Natural-absorption eval (no injection, first-letter proxy metric).** Residual weighting gives a small consistent improvement: mean coverage gap 0.768 → 0.735 (largest: 'h' 0.74 → 0.61, 's' 0.89 → 0.81), feature-splitting reduced (6.4 → 5.6 probe-aligned latents). Proxy-level, small-scale (500k tokens), stated as directional evidence only.
