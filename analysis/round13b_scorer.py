"""Round 13b (FROZEN AT LOCK): capacity-sweep scorer.

This is analysis/round13a_family_endpoint.py with the single-latent and family
counters COPIED VERBATIM, plus added `lost` counters (prereg P5: at small m the
SAE may drop the letter entirely rather than absorb it, and `absorbed` requires
`retained`). Adding a counter cannot change the existing ones, so 13a harness
fidelity -- which passed gate 4 against frozen round-12 values at max |d|=0.0017
-- carries over by construction.

Original 13a docstring follows.
