...
1. **A non-identifiability wall (Theorems 1, 1b).** When a child feature *never* occurs without its parent, the hierarchical ("faithful") and merged ("absorbed") feature ontologies generate *identical* data distributions — no method, present or future, can distinguish them from activations alone. Moreover, at this wall the absorbed configuration is the global optimum of the SAE objective over dictionaries of any size: the *active feature directions* are uniquely determined ({a_p, (a_p+a_c)/√2}; dictionary columns and codes remain non-unique under permutation, duplication with code-splitting, and unused atoms — two-line proof via ‖f‖₁ ≥ ‖Df‖). Absorption at the boundary is not a bug — but state the three claims separately (per the whole-repo review): (i) *data non-identifiability* — the two ontologies are observationally indistinguishable, so neither is privileged by the data (this is the theorem); (ii) *objective preference* — among those indistinguishable codings, the SAE's sparsity objective selects the absorbed one; (iii) *code identifiability* is a further, separate question (§16). Absorption is the objective's preferred coding of a non-identifiable distribution, **not** "the information-theoretic truth."
...
6. **The pre-registered coherence-penalty prediction was refuted, and the refutation became the deepest result (§7.1b).** The predicted critical penalty β\* misses the true global optimum: an **anti-rotated absorbed pair** — the composite keeps absorbing while the parent latent rotates to zero the pair's coherence. This generalizes to a **no-go theorem** (general-(d, m) derivation with proofs: `theory/general_no_go.md`). Evidence tiers, kept separate: *(proved)* every penalty that is a function of pairwise decoder inner products is constant on the manifold of orthonormal frames, so none — Frobenius, |cos|, OrtSAE-style max-cos², any future variant — can distinguish a faithful orthogonal frame from an anti-rotated absorbed one; *(analytic reduction)* the in-plane orthonormal-frame competition that turns this blindness into an absorption result holds for m ≤ d with non-pair latents off the pair plane; *(numerical)* the finite-β boundaries below and the absence of better mixed plane/background configurations come from exact scans and systematic search, not proof. Quantitatively (corrected, computationally verified): the penalty shrinks the absorption region at most ~4× (0.0486 → 0.0112 at β ≈ β\*), never eliminates it, worsens when overdosed (ε\*\*(β) increases to a penalty-form-independent limit ε\*\*(∞) ≈ 0.0159), and the absorbed configuration survives as a *local trap* up to ε ≈ 0.05 — nearly the original unpenalized boundary. GPU-validated at 160 runs (basin fractions cross at ≈ 0.6·ε\*\*; multistability persists exactly where the trap analysis predicts). The general derivation adds a **domain boundary**: this failure regime is precisely p₀ < p₀\* ≈ √2·q (parent-solo rarer than √2× co-occurrence — the case for all experiments here); for p₀ > p₀\*, the anti frame's 2-sparse parent-solo codes cost it the competition and coherence penalties genuinely work, even at ε = 0.

**Practitioner rule of thumb (a transition *scale*, not an exact threshold):** in the capacity-limited two-latent model, **1.17·λq is the characteristic scale of the absorption-to-faithfulness transition** — the exact crossover of the two pure candidate dictionaries. The continuously optimized two-latent dictionary passes its midpoint at ≈ 0.88·ε\* and SGD at 0.58–0.70·ε\* (§8), so a child occurring alone with probability ε ≪ 1.17·λq sits deep in the absorbed regime, and resolving it requires roughly **λ ≲ ε / (1.17 q)**. Scope: "more compute/data cannot help" refers to the *population optimum of this objective under the two-latent capacity constraint* — with spare capacity the same objective instead prefers the redundant triple (§4 Remark), and at ε = 0 the ontologies are indistinguishable outright (Theorem 1).
...
with equality iff the active latents all point exactly at x/r. Summing over the ε = 0 event distribution (assumptions: p₀, q > 0 and 0 < λ < 2, so r ≥ λ/2 holds for both event types), L(D) ≥ p₀(λ − λ²/4) + q(√2λ − λ²/4), and the bound is attained **iff the set of directions used with positive probability is exactly {a_p, a_m}, a_m = (a_p + a_c)/√2** — both directions must occur, since both event classes have positive probability. What is unique is that *set of active directions*; the dictionary and codes are not — multiple columns may occupy either direction with the coefficient split arbitrarily among them (preserving the total code), and additional columns may remain unused. *(Two corrections from the external-review thread: "unique optimum over every dictionary of every size" was too strong — unused atoms are free — and even "extra atoms stay unused" was too strong, since ‖f‖₁ is invariant to splitting a coefficient across duplicate collinear columns, so duplicates may be active.)* ∎
...
Methodologically: this correction exists because the prediction was pre-registered, the experiment was allowed to falsify it, and the falsification was chased into the exact model rather than absorbed into hand-waving. The computationally verified §7.1 algebra was *correct* — for the wrong configuration space.
...
![Empirical absorption transition (SGD)](figures/fig1_absorption_transition.png)

- **The wall behaves exactly as proven.** At ε = 0, all 15 runs across every (λ, q) land on the absorbed dictionary: mean φ = 45.75°, with a separate parent latent present in each — Theorem 1b's optimal active-direction set, found by SGD every time.
...
![Recovery phase diagram](figures/fig3_recovery.png)
...
**Coherence penalties as an absorption remedy are prior art — the critical-strength derivation and the anti-rotation obstruction are not.** OrtSAE (Korznikov et al., arXiv:2509.22033) adds a chunked max-pairwise-cosine decoder penalty explicitly against absorption and reports a 65% absorption reduction, with the weight (γ = 0.25) chosen empirically and no theorem or threshold anywhere. C²R's Eq. 13/14 is the nearest critical-condition analogue but is expressed in data-dependent latent energies. Against that backdrop, §7.1b contributes: the first *derived* critical strength β\*, the proof that it was the wrong question (anti-rotation evasion), the corrected bounded-and-non-monotone ε\*\*(β), and a falsifiable prediction about published methods — OrtSAE-style penalties should show a *bounded* absorption reduction that *degrades* when the penalty weight is pushed well past its sweet spot, because no Gram-type penalty can distinguish faithful orthogonal frames from anti-rotated absorbed ones. *(Prediction status, verified literature sweep 2026-07-23 — `notes/deep-research-2026-07-23-ortsae-and-round9-novelty.md`: the OrtSAE paper cannot test it — γ is fixed at 0.25 in every experiment, no γ ablation exists; its single operating point matches the bounded half (~65% reduction, absorption floor 0.05–0.095, 3–4× above Matryoshka's, with C²R independently confirming the floor). The closest published penalty-strength sweep, MetaSAEs arXiv:2604.03436, shows exactly the predicted non-monotonic shape in a neighboring penalty family: intermediate weights Pareto-optimal, largest weight worsening co-activation. The strong-form γ-sweep test on a fixed absorption metric remains an open external experiment; our own in-model γ sweep is §12's 320-run independent penalty-form check.)*
...
The overall contribution: the *failure side made exact* — a closed-form pure-strategy crossover with a computationally verified derivation, the objective-misalignment regime (absorption preferred under the two-latent capacity constraint despite identifiability), the corrected remedy theory, and SGD-level confirmation of each — plus one refuted pre-registration converted into the sharpest result.
...
The no-go theorem says dictionary-geometry penalties cannot work in the p₀ < √2·q regime; the code/event structure can, in every regime. The simplest remedy family that uses it — weight the per-sample loss by the event class — admits a complete exact analysis (`theory/remedy_weighted.py`):
...
Solving C(λ) = 1: **λ_crit = [(12 − 6√2) − √((12 − 6√2)² − 8)]/2 ≈ 0.714**. For every λ < λ_crit — i.e., every practically relevant sparsity coefficient — the faithful dictionary beats the absorbed one **for every ε > 0, however small**. Verified by global scans over all 1- and 2-latent dictionaries down to ε = 10⁻⁴ (faithful global optimum at every ε; unweighted control reproduces the vanilla transition). Scan boundaries sit at ~0.62× the pure-strategy closed forms because the scan classifier measures the 57°-crossing of the continuously tilting optimum, not the pure-strategy crossover — the same lead factor as the unweighted control; the w_j/w_c ratio scaling itself is confirmed exactly.
...
**Why this is the principled version of existing practice.** Weighted SAEs (arXiv:2506.15963) reweight reconstruction empirically; C²R (arXiv:2606.30609) conditions on activation energies. The results here say data-dependent weighting is not merely *a* remedy option — by the no-go theorem it is the only *kind* of remedy (among dictionary-geometry vs data-weighting families) that can work across the whole parameter space — dictionary-geometry penalties are provably blind below the √2 occurrence ratio, and inverse-density is the canonical instance with a clean optimality guarantee in the solvable model.
...
**What did not survive disambiguation (v1 claims withdrawn).** The v1 "residual weighting rescues 3/4 at ε = 0.002" was an artifact of a bare-probe functional metric that is out-of-distribution for the encoder; with realistic probes (real background + child injection), residual ≈ vanilla on both dictionary quality and routing (0.70 vs 0.64 routing at ε = 0.002 — marginal, seed-overlapping). The natural-absorption coverage-gap improvement **failed its selectivity control** (off-letter firing +27% relative under residual weighting — consistent with densification, not better features). **Oracle inverse-density weighting is genuinely harmful on real data** — worst dictionary alignment (0.68–0.79) and worst routing (as low as 0.07) on sound metrics: the §12 variance caveat is a real failure mode, not a theoretical nicety.

**The honest synthesis.** The weighted remedy's dramatic toy-model success (48/48 at 100× below the boundary) does not transfer *at these settings* because the real-data experiment sits in the spare-capacity **composition** regime, where harmful absorption — a capacity-limited phenomenon by Theorem 2's own scope — does not occur in injected-pair form. The remedy's real-data value is therefore **unproven, not refuted**: the decisive experiment is a capacity-*limited* real-data test (m small relative to the effective feature load, matching the regime of production SAEs on full-scale LLMs where Chanin-style absorption is actually observed). Queued as the next round.

**Weight-misestimation robustness (theory; unaffected).** k-fold rarity misestimation tolerated up to provable **k\* = 1/C(λ) = 4.12** (actual ≈ 6.6×), failure ε-independent; all formulas sympy-verified (`theory/remedy_robustness.py`, `theory/symbolic_verify.py`).
...
| **scarce capacity (m = 128/256)** | **true harmful absorption**: composite 0.99, child ≈ 0.74 = the composite's own shadow — no independent child latent | **rescue: child cos ≥ 0.99 in 6/8 seeds, 0.91 in one, one failed seed (0.20); composite drops to the child's shadow (0.71) — the faithful configuration** | ≈ vanilla (no rescue) |
...
### 15b. Label-free weighting, attempt 2: background-relative novelty — refuted, mechanism identified
