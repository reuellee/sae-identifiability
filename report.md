...
with equality iff the active latents all point exactly at x/r. Summing over the ε = 0 event distribution (assumptions: p₀, q > 0 and 0 < λ < 2, so r ≥ λ/2 holds for both event types), L(D) ≥ p₀(λ − λ²/4) + q(√2λ − λ²/4), and the bound is attained **iff every *active* atom points along a_p or a_m = (a_p + a_c)/√2**. What is unique is the *set of active directions*; the dictionary and codes are not — permutation, duplicate collinear columns with the coefficient split among them, and unused atoms all attain the same value. *(Two corrections from the external-review thread: "unique optimum over every dictionary of every size" was too strong — unused atoms are free — and even "extra atoms stay unused" was too strong, since ‖f‖₁ is invariant to splitting a coefficient across duplicate collinear columns, so duplicates may be active.)* ∎
...
![Absorption phase transition](figures/fig1_absorption_transition.png)
