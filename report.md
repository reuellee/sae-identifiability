...
- **A no-go theorem, penalty-form independent (`theory/no_go_theorem.py`).** Let the penalty be *any* function of pairwise decoder inner products with orthogonality optimal — Frobenius Gram, |⟨d_i,d_j⟩|, OrtSAE-style max-cos², all published coherence penalties. On the manifold of exactly orthonormal frames every such penalty is *constant*, so as β → ∞ the optimum is decided purely by reconstruction + L1 cost among orthonormal frames: the penalty's functional form drops out. Since the faithful frame {0°, 90°} and the anti-rotated absorbed frame {−45°, 45°} are both orthonormal, **no pairwise-coherence penalty, of any form, at any strength, can distinguish them** — the β → ∞ boundary ε\*\*(∞) is a penalty-form-independent constant.
...
**Honest caveats.** (i) Inverse-density weighting requires estimating event probabilities — trivial in the toy model, an open problem on real activations; (ii) upweighting rare events by 1/P inflates SGD variance as ε → 0 and would catastrophically amplify noise/outlier events in real data — a practical implementation needs clipping or annealing, whose theory is not done here; (iii) the sharp falsifiable prediction for GPU round 4: an inverse-density-weighted SAE at ε as low as ~10⁻³ (30× below the vanilla boundary, 10× below the best any coherence penalty achieves) should reliably recover the faithful child latent, with no multistability, because the absorbed basin does not exist in the weighted landscape.
