...
log "=== round 9 patient GPU retry (6h cap, 10min cycle) ==="
...
while [ $(date +%s) -lt $DEADLINE ]; do
  ...
  for z in europe-west4-a us-east1-c asia-southeast1-b; do
    gcloud compute instances create dev-gpu-2 --zone=$z --machine-type=g2-standard-8 --image=dev-gpu-img-tmp --boot-disk-size=150GB --maintenance-policy=TERMINATE --quiet >>~/retry_round9.log 2>&1
  ...
  log "all zones stocked out; sleeping 600s"
  sleep 600
done
