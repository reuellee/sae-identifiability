...
Output: SUMMARY3.md, results_capacity.csv, cap_done.flag.
"""
...
for M_LAT in (128, 256):
    ...
    is_re = torch.tensor([c == "residual" for c, _, _ in runs], device=dev)
...
with open("results_capacity.csv", "w", newline="") as fh:
    wcsv = csv.DictWriter(fh, fieldnames=list(rows_csv[0].keys()))
    wcsv.writeheader(); wcsv.writerows(rows_csv)
open("SUMMARY3.md", "w").write("\n".join(out))
open("cap_done.flag", "w").write("ok\n")
